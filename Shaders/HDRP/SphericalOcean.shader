...
Shader "SphericalOcean/HDRP"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // --- FFT cascade textures (set by SphericalOceanRenderer) ---
            TEXTURE2D(_OceanCascadeDisp0);  TEXTURE2D(_OceanCascadeDisp1);
            TEXTURE2D(_OceanCascadeDisp2);  TEXTURE2D(_OceanCascadeDisp3);
            TEXTURE2D(_OceanCascadeFoam0);  TEXTURE2D(_OceanCascadeFoam1);
            TEXTURE2D(_OceanCascadeFoam2);  TEXTURE2D(_OceanCascadeFoam3);
            SAMPLER(sampler_linear_clamp);
            int _OceanCascadeCount;
            float4 _OceanCascadeWeights;
            ...
            // ============================================================
            //  FFT DISPLACEMENT SAMPLING — cascaded, spherical UV mapping
            // ============================================================

            float3 SampleFFTDisplacement(float2 sphericalUV)
            {
                float3 disp = float3(0, 0, 0);
                float totalWeight = 0;

                // Sample up to 4 cascades, blend by distance-based weights
                if (_OceanCascadeCount > 0 && _OceanCascadeWeights.x > 0.001)
                {
                    float4 d = SAMPLE_TEXTURE2D_LOD(_OceanCascadeDisp0, sampler_linear_clamp, sphericalUV, 0);
                    disp += float3(d.x, d.y, d.z) * _OceanCascadeWeights.x;
                    totalWeight += _OceanCascadeWeights.x;
                }
                if (_OceanCascadeCount > 1 && _OceanCascadeWeights.y > 0.001)
                {
                    float4 d = SAMPLE_TEXTURE2D_LOD(_OceanCascadeDisp1, sampler_linear_clamp, sphericalUV, 0);
                    disp += float3(d.x, d.y, d.z) * _OceanCascadeWeights.y;
                    totalWeight += _OceanCascadeWeights.y;
                }
                if (_OceanCascadeCount > 2 && _OceanCascadeWeights.z > 0.001)
                {
                    float4 d = SAMPLE_TEXTURE2D_LOD(_OceanCascadeDisp2, sampler_linear_clamp, sphericalUV, 0);
                    disp += float3(d.x, d.y, d.z) * _OceanCascadeWeights.z;
                    totalWeight += _OceanCascadeWeights.z;
                }
                if (_OceanCascadeCount > 3 && _OceanCascadeWeights.w > 0.001)
                {
                    float4 d = SAMPLE_TEXTURE2D_LOD(_OceanCascadeDisp3, sampler_linear_clamp, sphericalUV, 0);
                    disp += float3(d.x, d.y, d.z) * _OceanCascadeWeights.w;
                    totalWeight += _OceanCascadeWeights.w;
                }

                return disp;
            }

            float SampleFFTFoam(float2 sphericalUV)
            {
                float foam = 0;
                float totalWeight = 0;
                ...
                if (_OceanCascadeCount > 0 && _OceanCascadeWeights.x > 0.001)
                {
                    foam += SAMPLE_TEXTURE2D(_OceanCascadeFoam0, sampler_linear_clamp, sphericalUV).r * _OceanCascadeWeights.x;
                    totalWeight += _OceanCascadeWeights.x;
                }
                if (_OceanCascadeCount > 1 && _OceanCascadeWeights.y > 0.001)
                {
                    foam += SAMPLE_TEXTURE2D(_OceanCascadeFoam1, sampler_linear_clamp, sphericalUV).r * _OceanCascadeWeights.y;
                    totalWeight += _OceanCascadeWeights.y;
                }
                if (_OceanCascadeCount > 2 && _OceanCascadeWeights.z > 0.001)
                {
                    foam += SAMPLE_TEXTURE2D(_OceanCascadeFoam2, sampler_linear_clamp, sphericalUV).r * _OceanCascadeWeights.z;
                    totalWeight += _OceanCascadeWeights.z;
                }
                if (_OceanCascadeCount > 3 && _OceanCascadeWeights.w > 0.001)
                {
                    foam += SAMPLE_TEXTURE2D(_OceanCascadeFoam3, sampler_linear_clamp, sphericalUV).r * _OceanCascadeWeights.w;
                    totalWeight += _OceanCascadeWeights.w;
                }

                return totalWeight > 0.001 ? foam : 0;
            }
            ...
            // ============================================================
            //  VERTEX SHADER — FFT + Gerstner hybrid, foam analytic
            // ============================================================
            ...
            Varyings Vert(Attributes v)
            {
                ...
                // --- FFT Displacement (from cascades) ---
                float3 fftDisp = float3(0, 0, 0);
                float fftFoam = 0;

                if (_OceanCascadeCount > 0)
                {
                    // Convert vertex direction to spherical UV for FFT texture sampling
                    float3 dir = normalize(v.vertex.xyz);
                    float u = 0.5 + 0.5 * atan2(dir.z, dir.x) / PI;
                    float v_uv = 0.5 - asin(dir.y) / PI;
                    float2 sphericalUV = float2(u, v_uv);

                    fftDisp = SampleFFTDisplacement(sphericalUV);
                    fftFoam = SampleFFTFoam(sphericalUV);
                }

                // --- Gerstner Waves (Burst job detail) ---
                GerstnerResult wave = GerstnerWave(tangentSpacePos, _CrestTime);

                // --- Hybrid Displacement: FFT (large/medium) + Gerstner (detail) ---
                float3 displacedPos = unpivotPos;

                // FFT displacement: apply as radial (height) + tangent (horizontal)
                displacedPos += sphereNormal * fftDisp.y;
                displacedPos += tangent * fftDisp.x;
                displacedPos += bitangent * fftDisp.z;

                // Gerstner displacement: add on top for high-frequency detail
                ...
                // --- Normals: combine FFT and Gerstner ---
                ...
                // Foam: blend FFT Jacobian foam with analytic Gerstner foam
                o.foamValue = max(fftFoam, wave.foamJacobian);
