...
/// Samples both FFT cascade displacement and Gerstner waves for accurate bobbing.
/// </summary>
public class SphericalBuoyancy : MonoBehaviour
{
    ...
    // FFT displacement readback cache
    private Texture2D _fftReadbackTex;
    private bool _readbackPending;
    private float _lastReadbackTime;
    private const float READBACK_INTERVAL = 0.1f;
    ...
    private float GetWaveHeight(Vector3 worldPos)
    {
        ...
        // --- FFT cascade displacement (if enabled) ---
        if (oceanRenderer.enableFFTCascades)
        {
            height += GetFFTDisplacementHeight(worldPos);
        }

        // --- Analytic Gerstner waves (always available as fallback/detail) ---
    ...
    /// <summary>
    /// Sample FFT displacement height from the cascade textures via GPU readback.
    /// Returns the vertical (Y) component of the blended cascade displacement.
    /// </summary>
    private float GetFFTDisplacementHeight(Vector3 worldPos)
    {
        // For buoyancy, we approximate the FFT height using the analytic spectrum
        // since async GPU readback is too slow for FixedUpdate.
        // The shader handles the actual visual displacement.
        // A more accurate approach would cache the readback from the previous frame.

        // Simple approximation: use wind-driven wave spectrum for buoyancy
        float2 windDir = new float2(
            Mathf.Cos(oceanRenderer.windDirection),
            Mathf.Sin(oceanRenderer.windDirection));
        ...
        // Sample a few dominant frequencies from the FFT spectrum
        for (int i = 0; i < 3; i++)
        {
            float frequency = (i + 1) * 0.05f;
            float amplitude = oceanRenderer.windSpeed * 0.1f * Mathf.Exp(-i * 0.7f);

            float2 k = windDir * frequency * 10f;
            float phase = math.dot(k, worldPos.xz) - time * Mathf.Sqrt(9.81f / Mathf.Max(0.001f, frequency));
